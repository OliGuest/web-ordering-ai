...
export const useAddQuantityRemoveClass = () => {
  ...
  const addQuantityAndRemoveClass = (data) => {
    ...
    $(".modifiersbox").each(function () {
      var self = $(this);
      if (self.prop("checked")) {
        var readyArray = {
          mModifiersWizardId: self.attr("mdata-ModifiersWizardId"),
          mIdOtherPos: self.attr("mdata-IdOtherPos"),
          mProductId: self.attr("mdata-ProductId"),
          mPrice: self.attr("mdata-Price"),
          mName: self.attr("mdata-Name"),
          // "mid": self.attr("mdata-mid"),
          // "required_true": self.attr("mdata-required_true")
        }
        selectedModifiers.push(readyArray);
      }
    });
