...
const PaymentModal = ({ showPaument, handlePaymentClose, handleShowPaymentMethod }) => {
    ...
    useEffect(() => {
        setUseServiceFee(subTotalServiceFee.UseServiceFee)
    }, [subTotalServiceFee]);
    ...
    return (
        <div >
            <Modal id="payment-modal"
                ...
                aria-hidden="true" show={showPaument} onHide={handlePaymentClose}
                centered
            >
                <Modal.Header >
                    <button onClick={handlePaymentClose}>
                ...
                <Modal.Body>
                    <div className="dynamic-list">
                        ...
                        <div className="bill-service-total">
                            <div className="menu-list-item d-flex" >
                                <div className="item-content ">
                                    <div className="item-inner-content add-qauntity d-flex">
                                        <div className="item-left-text mr-23 disabled centeredText  ">
                                            <h6 className="">{t("lblSubtotal")}:</h6>
                                        </div>
                                        <h6 className="item-price ml-auto">{subTotalServiceFee.StringTotal}</h6>
                                    </div>
                                    {UseServiceFee &&
                                        <div className="item-inner-content add-qauntity d-flex">
                                            <div className="item-left-text mr-23 disabled centeredText  ">
                                                <h6 className="">{t("lblServiceFee")} {(subTotalServiceFee.ServiceFeePercent + "%")}:</h6>
                                            </div>
                                            <h6 className="item-price ml-auto">{subTotalServiceFee.StringServiceFee}</h6>
                                        </div>
                                    }
                                    <div className="item-inner-content add-qauntity d-flex total">
                                        <div className="item-left-text mr-23 disabled centeredText  ">
                                            <h6 className="">{t("lblTotalPayment")}:</h6>
                                        </div>
                                        <p className="total-price ml-auto">{UseServiceFee ? subTotalServiceFee.StringTotalWithServiceFee : subTotalServiceFee.StringTotal}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <Modal.Footer>
                    <div className="bottom-clear-buttons">
                        <p className="btn-bottom clearAll-allergens"
                            onClick={handlePaymentClose}>{t("IbICancel")}</p>

                        <button className="btn-bottom apply-allergens" onClick={() => { handlePaymentClose(); handleShowPaymentMethod() }}>{t("lblContinueOrdering")}</button>
                    </div>
                </Modal.Footer>
            </Modal>
        </div>
