...
const Home = () => {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll(".main-menu-list .main-menu-item").forEach(item => {
  ...
  const errorScrollTo = () => {
    if (errorModifiers.length > 0) {
      ...
      const container = document.querySelector(".order-listing");
      const scrollTo = document.getElementById("ModifierId_" + firstErrorId);
      const elBackgroundColor = document.querySelector(`#ModifierId_${firstErrorId} .extra-features-list`);
      const labelEl = document.querySelector(`#ModifierId_${firstErrorId} .label-name`);
      const labelName = labelEl ? labelEl.innerText : '';
      if (container && scrollTo) {
        const topMenu = document.getElementById("top-menu");
        const divTopHeight = scrollTo.getBoundingClientRect().top - 270;
        const containerTop = container.getBoundingClientRect().top;
        const offset = divTopHeight - containerTop + container.scrollTop - (topMenu ? topMenu.offsetHeight : 0);
        container.scrollTo({ top: offset, behavior: 'smooth' });

        // Blink effect
        if (elBackgroundColor) {
          for (let i = 0; i < 3; i++) {
            setTimeout(() => {
              elBackgroundColor.classList.add("glow");
              elBackgroundColor.style.backgroundColor = "#ffe8e1";
              setTimeout(() => {
                elBackgroundColor.classList.remove("glow");
              }, 100);
            }, i * 200);
          }
        }
  ...
  const centeringCard = () => {
    const container = document.querySelector(".order-listing");
    const scrollTo = document.querySelector(`.${clickedCardId}`);
